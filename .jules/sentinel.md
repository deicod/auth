...
## 2026-02-14 - IP Spoofing via Split X-Forwarded-For Headers
**Vulnerability:** The application used `r.Header.Get("X-Forwarded-For")` which only retrieves the first header line. If a proxy adds a *new* header line instead of appending to the existing one (e.g., `X-Forwarded-For: spoofed` \n `X-Forwarded-For: real`), the application would process only the spoofed header and ignore the real IP.
**Learning:** HTTP headers like `X-Forwarded-For` can appear multiple times. `Header.Get()` in Go is insufficient for security checks as it only returns the first value. Security critical logic must iterate over *all* values of the header (`r.Header[...]`).
**Prevention:** Modified `clientIP` to iterate over `r.Header["X-Forwarded-For"]` in reverse order, ensuring the full chain of proxies is inspected regardless of how the headers are split.
