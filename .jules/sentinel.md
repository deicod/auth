...
## 2026-02-09 - IP Spoofing via X-Forwarded-For Trust-First Parsing
**Vulnerability:** The application parsed the `X-Forwarded-For` header by taking the **first** (leftmost) IP address if the immediate peer was trusted. This is vulnerable to IP spoofing because standard proxies append the client IP to the existing header. An attacker could inject a fake IP (e.g., `X-Forwarded-For: spoofed`), and the proxy would append the real IP (`spoofed, real`). The application, trusting the proxy, would incorrectly use the `spoofed` IP, bypassing rate limits and audit logs.
**Learning:** When using `X-Forwarded-For`, the "leftmost" IP is only the original client *if* the header was generated by a trusted source that *replaces* the header. In most append-only proxy chains, the only trusted IP is the one added by the *last trusted proxy*. Therefore, verification must proceed from **right to left**, backtracking through the chain of trusted proxies until the first untrusted IP is found.
**Prevention:** I refactored the `clientIP` logic to iterate backwards through the `X-Forwarded-For` list. It now skips IPs that match the configured `TrustedProxies` list and returns the first untrusted IP encountered (which represents the true client IP entering the trusted network).

## 2026-02-16 - IP Spoofing via Multiple X-Forwarded-For Headers
**Vulnerability:** The application used `r.Header.Get("X-Forwarded-For")` to retrieve the client IP chain. `Get()` only returns the first header value. If a proxy adds a *new* header line instead of appending to the existing one (valid per RFC 2616), an attacker could send a spoofed header (e.g., `X-Forwarded-For: spoofed`) which the application would see as the only header, ignoring the real IP added by the proxy in a second header.
**Learning:** `http.Header.Get()` is insufficient for multi-valued headers where all values matter (like `X-Forwarded-For`). Security-critical header parsing must consider all values, typically by joining them or iterating through all slice elements.
**Prevention:** I modified `clientIP` in `handlers/auth.go` to use `strings.Join(r.Header["X-Forwarded-For"], ", ")`. This ensures the full proxy chain is constructed and validated from right to left, preventing the spoofed IP from being accepted as the client IP.

## 2026-02-17 - DoS via Synchronous Email Sending
**Vulnerability:** The `InitiateEmailChange` endpoint performed two synchronous email sending operations (alert to old email, confirmation to new email). This blocked the HTTP handler for the duration of the SMTP transaction, allowing attackers to exhaust server resources (worker threads/connections) by holding connections open.
**Learning:** Network I/O, especially SMTP, is slow and unpredictable. Synchronous execution of such tasks within the request lifecycle creates a denial-of-service vector where a small number of concurrent requests can saturate the server's capacity.
**Prevention:** I refactored `InitiateEmailChange` to perform email sending asynchronously using a goroutine with a detached context and timeout. This ensures the HTTP handler returns immediately, freeing up resources while the email is sent in the background.

## 2026-02-18 - Missing Authentication on Email Change Initiation
**Vulnerability:** The `InitiateEmailChange` endpoint accepted a `user_id` in the request body and did not require an active session token. This allowed any attacker who obtained a user's password (e.g., via phishing or credential stuffing) to initiate an email change for that user without logging in, bypassing session-based security controls (like IP restrictions or "new device" alerts). It also facilitated IDOR attacks if the attacker could guess valid User IDs.
**Learning:** Sensitive account operations (like changing email or password) must strictly require an active, authenticated session. Relying solely on re-authentication (password check) within the request body is insufficient because it bypasses the trusted context established by the session.
**Prevention:** I updated the `InitiateEmailChange` handler to require a valid Bearer token. The handler now authenticates the session using `AuthenticateSession` and derives the target `UserID` directly from the session, ignoring the `user_id` provided in the request body. This ensures that only the currently logged-in user can initiate changes for their own account.

## 2026-02-25 - Insecure Default for TrustedProxies
