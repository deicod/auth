...
## 2026-02-12 - IP Spoofing via Multiple X-Forwarded-For Headers
**Vulnerability:** The `clientIP` logic used `r.Header.Get("X-Forwarded-For")` which only returns the first header line. If a proxy appended a new `X-Forwarded-For` header instead of merging it into the existing one, the application would only see the first header (potentially attacker-controlled) and fail to validate the full chain, allowing IP spoofing.
**Learning:** In Go, `Header.Get()` is insufficient for list-type headers like `X-Forwarded-For` when security is involved. Multiple header lines are valid according to RFCs, and attackers can exploit this ambiguity if the downstream proxy behavior (appending vs merging) doesn't match the application's parsing logic.
**Prevention:** I updated the logic to fetch all `X-Forwarded-For` values using `r.Header["X-Forwarded-For"]` and join them with a comma before parsing. This ensures the full proxy chain is visible and validated regardless of whether the proxy merges or appends headers.
